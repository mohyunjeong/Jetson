...
void setup() {
  ...
  pinMode(SCL_PIN, OUTPUT); // 출력 모드
  pinMode(SDO_PIN, INPUT); // 입력 모드
}
...
byte Read_Keypad(void) {
  ...
  for (byte Count = 1; Count <= 16; Count++) { // 16까지 사용할 거면 연결을 해줘야 함!
    digitalWrite(SCL_PIN, LOW);                // SCL_PIN 핀을 LOW 상태로 설정해서 현재 열을 활성화
